...
다음날부터 2주 후까지의 가격을 제공한다고해서 Tomorrow to Twentydays 즉, TT Market 입니다. <br />
일조량, 강수량, 온도, 기존 시세 등을 분석하여 농산물 가격 변동을 예측하는 알고리즘을 파이썬과 머신러닝으로 구현하였습니다.<br />
그렇게 구현한 메타 데이터들로 실시간 가격 변동 및 예측 정보를 제공하여 사용자들이 더 합리적인 구매 결정을 내릴 수 있도록 합니다. <br />
...
<details>
<summary>프로젝트 관리 도구의 효율적 활용</summary>
<br />

프로젝트 관리를 위해 Notion과 Slack을 활용하여 회의, 변경사항의 이력관리, To-Do 리스트 관리 등을 작성하였습니다. <br />

이를 통해 팀원들이 다른 팀원의 작업 진행도를 파악할 수 있게 되었고, <br />

변경점이 있을 때마다 알람이 가게 함으로써 협업과 소통능력을 높였습니다. <br />

이를 통해 프로젝트의 진행 상황을 효과적으로 관리할 수 있었습니다. <br />

</details>

<details>
<summary>데이터 조회 로직 최적화</summary>
<br />

주문 목록, 배송 목록 등 데이터를 찾을 때 시간이 오래 걸렸습니다. <br />

사용하였던 방식으로는 이중 for 문으로 특정 도메인에서 데이터를 찾고, 그 데이터를 기준으로 또 다른 도메인으로 넘어가 다음으로 필요한 데이터를 찾는 작업이었습니다. <br />

기존에 복잡했던 알고리즘 로직을 개선하기 위해 Index나 반정규화를 통해 개선하려고 하였습니다. <br />

시도했던 여러 방법 중에 데이터를 찾는 로직을 set으로 고치는 것이 가장 효과가 컸습니다. <br />

이로 인해 데이터당 시연 시간을 95ms에서 46ms로 줄였으며, 시간복잡도를 O(n^2)에서 O(n)으로 개선하여 더욱 시간을 적게 들게 했습니다. <br />


</details>

<details>
<summary>결제 정보의 효율적 관리와 트래픽 최소화</summary>
<br />

카카오 결제에선 결제를 요청과 확정하는 과정에서 서버에 동일한 결제 정보와 소비자 정보를 두 번 받을 필요가 있었습니다. <br />

저는 이 과정에서 불필요한 트래픽이 일어난다고 생각하여 트래픽을 최소화하고자 하였습니다. <br />

처음 결제 요청 과정에서 결제 정보와 소비자 정보를 받았을 때, 결제 정보를 Redis에 저장합니다. <br />

그 후, 결제를 확정하는 과정에서 소비자 정보만 받아 저장된 결제 정보를 찾고 결제 확정을 하도록 하였습니다. <br />

이런 과정을 통해 웹과 서버 간의 데이터 트래픽을 줄이고자 노력하였습니다. <br />

그리고 결제 정보가 Redis에 저장되었을 때 자동 소멸 주기를 설정하였습니다. <br />

이는 결제에 예외가 발생하여도 외부 결제 서버에서 한 번, Redis에서 한 번, 서버에 저장할 때 @Transactional에서 한 번 예외 처리가 되기 때문에 예외 처리 측면에서도 좋은 성과를 냈다고 생각합니다.<br />
